...
\begin{itemize}
	...
	\item \textbf{BLE gatttool interaction}: gatttool is one of the best methods to interact with another BLE device interactively from a Linux host. It opens an interactive session.\\
		...
		\end{figure}
		Open interactive session using gatttool:\\
		\textbf{sudo gatttool -i hci0 -b MAC\_ADDR -I}\\
		In the interactive sesssion type in the following commands:
		\textbf{connect}\\
		Read profiles:
		\textbf{primary}\\
		Read all available characterstics:\\
		\textbf{char-desc}\\
		\begin{figure}[ht]
			\centering
		...
		\begin{figure}[ht]
			\centering
			\includegraphics[scale=0.5]{images/gatttool12.png}
		...
		\end{figure}
    		...
    		Quit:\\
        	\textbf{quit}\\
	\item \textbf{Reading Beacons using BLE}: There is a python script that can be used for this purpose, install it:\\
		\textbf{git clone https://github.com/switchdoclabs/BeaconAirPython.git}\\
		In order to use, the following package must be installed:
		\textbf{sudo apt-get install python-bluez}\\
		Scan and Read the beacons:\\
		\textbf{cd BeanconAirPython/ble}\\
		\textbf{sudo python testblescan.py}\\
		This should list out the nearby beacons:
		\begin{figure}[ht]
			\centering
	...
	\item \textbf{Advertising with Beacons}:
		Start advertising in non-connectable state:\\
		\textbf{sudo hciconfig hci0 leadv 3}\\
		\textbf{sudo hciconfig hci0 noscan}\\
		Broadcast a beacon:
		\textbf{        python -c 'import sys,uuid;sys.stdout.write(uuid.uuid4().hex)'|xclip -selection clipboard && xclip -selection clipboard -o && echo
}\\
